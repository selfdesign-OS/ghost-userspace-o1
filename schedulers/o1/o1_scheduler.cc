...
void O1Scheduler::O1Schedule(const Cpu& cpu, BarrierToken agent_barrier,
  ...
  O1Task* next = nullptr;
  if (cs->preempt_curr) {
    GHOST_DPRINT(1, stderr, "Preempting current task %s on cpu %d", 
...
void O1Rq::EnqueueExpired(O1Task* task) {
    GHOST_DPRINT(1,stderr,"[enqueue Expried] task id : %s remaining Time : %lli enqueue Expried",task->gtid.describe(),absl::ToInt64Nanoseconds(task->remaining_time));
